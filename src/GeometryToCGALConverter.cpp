		      ...
		      std::vector<int>& vertices, bool print=false)
{
  ...
  if (print)
  {
    dolfin::cout << "Begin facet " << facet_no << dolfin::endl;
    if (!vertices.size())
    {
      dolfin::cout << "No vertices in facet!" << dolfin::endl;
      return;
    }

    // Print vertices
    for (std::vector<int>::iterator it=vertices.begin(); it != vertices.end(); it++)
      dolfin::cout << "Vertex: " << (*it) << dolfin::endl;

    if (builder.test_facet(vertices.begin(), vertices.end()))
      dolfin::cout << "Facet ok, size: " << vertices.size() << dolfin::endl;
    else
      dolfin::cout << "Facet not ok" << dolfin::endl;
  }
  ...
  if (print)
    dolfin::cout << "End facet" << dolfin::endl;
  facet_no++;
}
//-----------------------------------------------------------------------------
                                      ...
                                      bool remove_degenerated)
{
  ...
  {
    dolfin::cout << "Removing degenerated facets" << dolfin::endl;
